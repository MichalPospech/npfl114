...
class Network:
    def __init__(self, args, num_words, num_tags, num_chars):
        ...
        def conv_pool(width, in_tensor):
            tmp = tf.keras.layers.Conv1D(args.cnn_filters, width, strides= 1, padding = "valid", activation = 'relu')(in_tensor)
            pool = tf.keras.layers.GlobalMaxPool1D()(tmp)
            return pool

        convs = tf.keras.layers.Concatenate()([conv_pool(width, chars_embedding) for width in range(2,args.cnn_max_width+1)])

        char_embeds = tf.keras.layers.Dense(args.we_dim, activation = 'relu')(convs)
    ...
    def train_batch(self, inputs, tags):
        ...
        with self._writer.as_default():
            for name, metric in self._metrics.items():
                ...
                tf.summary.scalar("train/{}".format(name), metric.result(), step = None)
    ...
    def evaluate(self, dataset, dataset_name, args):
        ...
        with self._writer.as_default():
            for name, value in metrics.items():
                tf.summary.scalar("{}/{}".format(dataset_name, name), value, step = None)
