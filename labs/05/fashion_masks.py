...
class Network():
    def __init__(self, args):
        ...
        self._label_loss = tf.keras.losses.CategoricalCrossentropy(label_smoothing = 0.1)
        self._mask_loss = tf.keras.losses.BinaryCrossentropy()
        self.metrics = [[tf.keras.metrics.CategoricalAccuracy()], [
            comp_iou_2]]
        self._writer = tf.summary.create_file_writer(
            flush_millis=1000, logdir=args.logdir)
    ...
    def train(self, fashion_masks, args):
        for epoch in range(args.epochs):
            ...
            label_acc, mask_acc =self.evaluate(fashion_masks.dev, args)
            with self._writer.as_default():
                tf.summary.scalar("test/label_acc", label_acc, step=None)
                tf.summary.scalar("test/mask_acc", mask_acc, step=None)
            # Print development evaluation
            print('\rEpoch #{}, label accuracy: {:.4f}, iou: {:.4f}'.format(
                epoch+1, label_acc, mask_acc))
    ...
    def train_on_batch(self, images, labels_gold_cat, masks_gold):
        ...
        with self._writer.as_default():
            tf.summary.scalar("train/label_loss", label_loss, step=None)
            tf.summary.scalar("train/mask_loss", mask_loss, step=None)
