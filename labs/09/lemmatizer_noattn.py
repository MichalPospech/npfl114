...
class Network:
    ...
    def train_batch(self, source_charseq_ids, source_charseqs, target_charseq_ids, target_charseqs):
        ...
        with tf.GradientTape() as tape:
            ...
            source_states = self._model.source_rnn(embeds)
            ...
            source_states = tf.boolean_mask(
                tf.gather(source_states, source_charseq_ids), source_mask)
    ...
    def predict_batch(self, source_charseq_ids, source_charseqs):
        ...
        class DecoderPrediction(decoder.BaseDecoder):
            @property
            # TODO(train_batch): Return the batch size of self._source_states, using tf.shape
            def batch_size(self): 
                return tf.shape(self._source_states)[0]
            @property
            # TODO: Return 1 because we are returning directly the predictions
            def output_size(self): return 1
            @property
            # TODO: Return tf.int32 because the predictions are integral
            ...
            def step(self, time, inputs, states):
                ...
                finished = tf.math.equal(
                    outputs, MorphoDataset.Factor.EOW)
                return outputs, states, next_inputs, finished

        predictions, _, _ = DecoderPrediction(maximum_iterations=tf.shape(
            source_charseqs)[1] + 10)([self._model, source_states])
        return predictions
