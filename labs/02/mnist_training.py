...
num_batches = (mnist.train.size*args.epochs)/args.batch_size

optimizer_ctor = tf.optimizers.Adam if args.optimizer == "Adam" else tf.optimizers.SGD
parameters = dict()
if args.decay is None:
    parameters["learning_rate"] = args.learning_rate
elif args.decay == "polynomial":
        parameters["learning_rate"] = tf.keras.optimizers.schedules.PolynomialDecay(
            args.learning_rate, num_batches, end_learning_rate=args.learning_rate_final)
else:
    parameters["learning_rate"] = tf.keras.optimizers.schedules.ExponentialDecay(args.learning_rate,num_batches,args.learning_rate_final/args.learning_rate)
if args.optimizer =="SGD" and args.momentum is not None:
    parameters["momentum"] = args.momentum


model.compile(
    optimizer=optimizer_ctor(**parameters),
